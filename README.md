...
The project is configured for seamless deployment on free-tier platforms:

### 1. Unified Backend + Frontend on Render (Recommended)
You can deploy the app as a single Render Web Service using the provided [`render.yaml`](render.yaml) blueprint:
1. Connect your GitHub repository to **Render**.
2. Click **New** > **Blueprint Route** and select the repository.
3. Render will automatically parse [`render.yaml`](render.yaml), build dependencies, compile FAISS vector indexes on-the-fly, and host the FastAPI server serving the frontend statically.
4. Add the required environment secrets on Render's dashboard:
   * `GEMINI_API_KEY`
   * `SARVAM_API_KEY`

#### Deployed URLs:
* **Production Live URL**: `https://hh-goa-voice-radar.onrender.com`
* **Alternate/Static Frontend**: Can be hosted on Vercel/Netlify and automatically maps API endpoints to the Render backend via base URL resolution in `app.js`.

#### Free-Tier Constraints:
* **Render Free Web Service**: Spins down after 15 minutes of inactivity. The first request after spin-down incurs a cold-start boot delay of ~50 seconds.
* **Gemini Free Tier API**: Subject to a rate limit of 15 Requests Per Minute (RPM).
...
- [AUDIT_REPORT.md](AUDIT_REPORT.md) — Run 3 Requirements compliance report